...
float f(uint index)
{
    ...
    {
        //data += _A[uint2(index, i)].r * _MainTex[OFFSET_NR_VECTOR + uint2(i, 0)].r + _B[uint2(index, i)].r * (_MainTex[OFFSET_NR_VECTOR + uint2(i, 0)].r - _MainTex[OFFSET_OUT_BUFFER + uint2(i, 0)].r) / _DeltaTime;
        data += _A[uint2(index, i)].r * _MainTex[OFFSET_NR_VECTOR + uint2(i, 0)].r + _B[uint2(index, i)].r * (_MainTex[OFFSET_XDOT_VECTOR + uint2(i, 0)].r);
        
        exponent += _C[uint2(index, i)].r * _MainTex[OFFSET_NR_VECTOR + uint2(i, 0)].r;
    }
...
float dfidxj(uint i, uint j)
{
    ...
    {
        exponent += _C[uint2(i, k)].r * _MainTex[OFFSET_NR_VECTOR + uint2(k, 0)].r;
    }
