...
初期実装を `Scene.unity` の既存Breadboardに組み込み済み。既存の `model/Breadboard.fbx` を使用し、400穴の配置定義、Prefabによる部品の動的表示、操作パネル、JSON同期、MNA入力生成を分離している。
...
- [部品Prefabと基底クラス](Docs/DynamicParts.md)：生成・更新の契約と、モデル導入時の拡張箇所。
